...
for key in DEEPHPI_APP_URL DEEPHPI_MAIL_FROM DEEPHPI_SMTP_HOST DEEPHPI_SMTP_PORT DEEPHPI_SMTP_USER DEEPHPI_SMTP_PASS DEEPHPI_SMTP_TLS DEEPHPI_SMTP_SSL; do printf -v "${key}" '%s' "$(current_value "${key}")"; done
if prompt_yes_no 'Configure SMTP email notifications now?' false; then
    prompt_value DEEPHPI_APP_URL 'Public deepHPI URL including base path' "${DEEPHPI_APP_URL:-}" true
    prompt_value DEEPHPI_MAIL_FROM 'Notification sender address' "${DEEPHPI_MAIL_FROM:-}" true
    prompt_value DEEPHPI_SMTP_HOST 'SMTP host' "${DEEPHPI_SMTP_HOST:-}" true
    prompt_value DEEPHPI_SMTP_PORT 'SMTP port' "${DEEPHPI_SMTP_PORT:-587}"
    prompt_value DEEPHPI_SMTP_USER 'SMTP username (blank when relay authentication is not required)' "${DEEPHPI_SMTP_USER:-}" false
    prompt_secret DEEPHPI_SMTP_PASS 'SMTP password (blank when relay authentication is not required)' false
    prompt_value DEEPHPI_SMTP_TLS 'Require STARTTLS (true/false)' "${DEEPHPI_SMTP_TLS:-true}"
    prompt_value DEEPHPI_SMTP_SSL 'Use implicit TLS (true/false)' "${DEEPHPI_SMTP_SSL:-false}"
fi
